...
export const PlannerView = ({ state, updateState }: ViewProps) => {
   ...
   const [backlogRecurrence, setBacklogRecurrence] = useState<RecurrenceChoice>('none');
   const [dayModalRecurrence, setDayModalRecurrence] = useState<RecurrenceChoice>('none');
   ...
   const normalizedAnchor = useMemo(() => normalizeDate(anchorDate), [anchorDate]);
   ...
   const currentSelectedDate = useMemo(
      () =>
         isWithinRange(selectedDate, rangeStartKey, rangeEndKey) ? selectedDate : rangeStartKey,
   ...
   const bucketKeyForTask = (task: Task) => taskEffectiveDate(task) ?? UNSCHEDULED_BUCKET;
   ...
   const reorderTaskRelative = (sourceId: string, targetTask: Task) => {
      updateState((prev) => {
         ...
         const targetDateValue = targetBucket === UNSCHEDULED_BUCKET ? null : targetBucket;
   ...
   const addBacklogTask = () => {
      ...
      const cadence = backlogRecurrence === 'none' ? undefined : backlogRecurrence;
      const startDate = backlogDueDate || todayDateKey;
      const dates =
         cadence && backlogDueDate ? recurrenceDatesForYear(startDate, cadence) : [startDate];
      ...
      updateState((prev) => {
         ...
         const newTasks: Task[] = dates.map((dateKey, index) => {
            const monthTag = cadence
               ...
               : nowMonthLabel();
            const dueDateValue = cadence ? dateKey : backlogDueDate || undefined;
            return {
               id: generateId(),
   ...
   const addTaskForModalDate = () => {
      ...
      const cadence = dayModalRecurrence === 'none' ? undefined : dayModalRecurrence;
      const dates = cadence ? recurrenceDatesForYear(dayModalDate, cadence) : [dayModalDate];
      const recurrenceGroupId = cadence ? generateId() : undefined;
      updateState((prev) => {
         const orderStart = prev.tasks.length;
         return {
   ...
   const combineTasksWithPlaceholders = (tasks: Task[], bucketKey: string) => {
      const placeholders = deletedPlaceholders.filter((item) => item.bucketKey === bucketKey);
      const combined = [
         ...tasks.map((task) => ({
            kind: 'task' as const,
               ...
               <div className="absolute right-3 top-3 sm:hidden" data-task-menu-id={task.id}>
                  <div className="relative">
                     ...
                     {openTaskMenuId === task.id ? (
                        ...
                        >
                           <label
                              className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 text-left text-slate-700"
                              data-task-menu-id={task.id}
                           >
   ...
   const isRecurringWithoutStartDate = backlogRecurrence !== 'none' && !backlogDueDate;
      ...
      <>
                     ...
                     <div className="rounded-2xl bg-slate-100 p-1">
                        <button
                           className={`rounded-xl px-4 py-2 text-sm font-semibold transition cursor-pointer hover:-translate-y-0.5 ${
                              mode === 'week' ? 'bg-white text-slate-900 shadow' : 'text-slate-500'
                           }`}
                        ...
                        <button
                           className={`rounded-xl px-4 py-2 text-sm font-semibold transition cursor-pointer hover:-translate-y-0.5 ${
                              mode === 'month' ? 'bg-white text-slate-900 shadow' : 'text-slate-500'
                           }`}
                  ...
                  {mode === 'week' ? (
                     <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
                        {weekDates.map((dateKey) => {
                           ...
                           const displayLabel = parseDateKey(dateKey).toLocaleDateString(
                              undefined,
                              {
                                 weekday: 'short',
                                 month: 'short',
                        ...
                        {monthCalendarRows.map((row, idx) => (
                           <div key={idx} className="grid grid-cols-2 md:grid-cols-7 gap-2">
                              {row.map((day) => {
                                 ...
                                 const displayLabel = parseDateKey(day.key).toLocaleDateString(
                                    undefined,
                                    {
                                       weekday: 'short',
                                       month: 'short',
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
               ...
               <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl bg-slate-50 p-4 text-center">
                     ...
                     <p className="text-3xl font-semibold text-slate-900">{visibleTasks.length}</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 p-4 text-center">
                     <p className="text-xs uppercase tracking-wide text-slate-500">Unscheduled</p>
               ...
               <div className="mt-6 space-y-4">
                              ...
                              >
                                 ...
                                 {isExpanded && (
                                    <div className="mt-3 space-y-2">
                                       {combineTasksWithPlaceholders(dayTasks, dateKey).map(
                                          (item) =>
                                             item.kind === 'task'
                  ...
                  {(groupedTasks.unscheduled.length > 0 ||
                     ...
                     >
                        <div className="flex items-center justify-between">
                           <p className="text-sm font-semibold text-slate-900">Unscheduled tasks</p>
                           <span className="text-xs text-slate-500">
                              {groupedTasks.unscheduled.length}{' '}
         ...
         {dayModalDate && (
            <div className="fixed inset-0 z-40 flex items-center justify-center px-4">
               <div className="absolute inset-0 bg-slate-900/40" onClick={closeDayModal} />
               <div className="relative z-10 w-full max-w-3xl rounded-3xl bg-white p-6 shadow-xl">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                     <div>
                        <p className="text-sm uppercase tracking-wide text-slate-500">Tasks for</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                     ...
                     <select
                        ...
                        onChange={(e) => setDayModalRecurrence(e.target.value as RecurrenceChoice)}
                     >
               ...
               <div className="relative z-10 w-full max-w-lg rounded-3xl bg-white p-6 shadow-xl">
                  ...
                  <div className="mt-5 space-y-3">
                     ...
                     <select
                        ...
                        onChange={(e) => setBacklogRecurrence(e.target.value as RecurrenceChoice)}
                     >
                        {recurrenceOptions.map((option) => (
                           <option key={option.value} value={option.value}>
                              {option.label}
